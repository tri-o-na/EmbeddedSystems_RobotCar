...
static const float MAX_DETECTION_RANGE_CM = 400.0f;  // Maximum detection range for HC-SR04
...
float get_distance_cm(uint trigPin, uint echoPin) {
    ...
    // Return -1 only if beyond sensor's physical max range
    if (distance > MAX_DETECTION_RANGE_CM) {
        return -1.0f;  // Beyond sensor range
    }
...
// Stable distance with averaging filter
float get_stable_distance_cm(uint trigPin, uint echoPin) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        // Accept all valid readings from the sensor
        if (dist > 0 && dist <= MAX_DETECTION_RANGE_CM) {
...
// Median filter for stable distance measurement
